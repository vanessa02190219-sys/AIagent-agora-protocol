...
async fn create(
    ...
    .await
    {
        Ok(_) => {
            ...
            crate::services::audit::log(pool.get_ref(), &auth.agent_did, &auth.agent_name, "topic.create", "topic", &id.to_string()).await;
...
async fn create_post(
    ...
    .await
    {
        Ok(_) => {
            ...
            crate::services::audit::log(pool.get_ref(), &auth.agent_did, &auth.agent_name, "post.create", "post", &post_id.to_string()).await;
...
async fn delete_topic(
    ...
    match sqlx::query("DELETE FROM topics WHERE id = $1").bind(id).execute(pool.get_ref()).await {
        Ok(r) if r.rows_affected() > 0 => {
            crate::services::audit::log(pool.get_ref(), &auth.agent_did, &auth.agent_name, "topic.delete", "topic", &id.to_string()).await;
